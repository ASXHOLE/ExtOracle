Ext.onReady(function() {
	...
	var foreignlanguage=[['f1','英语一班'],['f2','英语二班'],['f3','日语一班'],['f4','日语二班']];	
	
	var machinecar=[['m1','车辆一班'],['m2','车辆二班'],['m3','车辆三班']];	
	
	var computer=[['c1','软工一班'],['c2','软工二班'],['c3','数媒一班'],['c4','计科一班'],['c5','计科二班']];
		...
		buttons:[
			...
			handler: function(){
				...
				for(var i=0;i<rowIndex;i++){
					...
					var fieldName = gp.getColumnModel().getDataIndex(1);  
					var data = record.get(fieldName);  
					if(Ext.getCmp('number').getValue()==data){
						Ext.MessageBox.alert("警告","该学生学号重复，不能添加！");
						return 0;
					}
				}
				/*添加新纪录*/
	...
	var checkedPost='';
	//获取复选组的值
     Ext.getCmp('postCheck').on('change', function (cbgroup, checked) {
     	checkedPost='';
	...
	var gp = new Ext.grid.EditorGridPanel({
        ...
        {  
            ...
            handler: function(){  
            	if(selModel.getSelections().length < 1 ){
        			Ext.MessageBox.alert('系统提示','请至少选择一条数据');
        			return;
       			}else{
       				Ext.MessageBox.confirm('确认操作','<span style="font-size:16px;color:#ff0000">警告</span> : 该操作将会删除选中的用户相关信息，且操作无法恢复。<br/>确定要继续吗？',
       					function(v){
	                 		if(v=='yes'){
	                  			var recs = gp.getSelectionModel().getSelections();
	                  			Ext.each(recs, function(item) {    
                                    jsonstore.remove(item);    
		...
		columns:[selModel,
		         ...
		         {header:"年龄",dataIndex:"age",editor:new Ext.form.NumberField()},
		         {header:"性别",dataIndex:"gender",
		         				renderer:function(value, metaData, record, rowIndex, colIndex, store) {  
        									var idx = sexStore.find("valField", value);  
        									return (idx != "-1") ? sexStore.getAt(idx).data.displayText : '';},//翻译性别代码
				...
				{header:"学院",dataIndex:"college",
		         				renderer:function(value, metaData, record, rowIndex, colIndex, store) {  
        									var idx = collegeStore.find("valueField", value);  
        									return (idx != "-1") ? collegeStore.getAt(idx).data.displayField : '';},//翻译学院代码
		         				editor: new Ext.form.ComboBox({  
                							editable: false,  
                							valueField: 'valueField',  
        									displayField: 'displayField',
				...
				{header:"班级",dataIndex:"class",
		         				renderer:function(value, metaData, record, rowIndex, colIndex, store) {  
        									...
        									Ext.getCmp('gender').getValue().getGroupValue();
		...
		store:jsonstore,
		listeners:{  
			'cellclick':function(grid, rowIndex, columnIndex, e){  //获取鼠标点击的该单元格该行的学院单元格的值
				//editCell_row = rowIndex;   
				//editCell_col = columnIndex;  
				var record = grid.getStore().getAt(rowIndex);  
				var fieldName = grid.getColumnModel().getDataIndex(5);  
				var data = record.get(fieldName);  
								...
								if(obj.field=='number'){
									...
									for(var i=0;i<rowCount;i++){
										if(i!=obj.row){
											...
											var data = record.get(fieldName);  
											if(obj.value==data){
												Ext.MessageBox.alert("警告","该学生学号重复，不能修改！");
												obj.record.set('number',obj.originalValue);
												return 0;
											}
										}
									}
